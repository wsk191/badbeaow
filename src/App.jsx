...
export default function BadmintonApp() {
  ...
  const handleWin = async (winnerTeam) => {
    ...
    try {
      ...
      showToast('บันทึกผลการแข่งขันไม่สำเร็จ', 'error');
    }
    setIsProcessing(false);
  };
          ...
          <div className="p-4 space-y-5">
            ...
            {!isAdmin && (
              <div className="bg-amber-50 border border-amber-200 p-4 rounded-2xl flex items-center justify-between">
                ...
                <button onClick={() => setShowLoginModal(true)} className="bg-amber-600 text-white text-xs font-bold px-3 py-2 rounded-xl shadow-sm hover:bg-amber-700">
                  ล็อกอินแอดมิน
                </button>
              </div>
            )}
